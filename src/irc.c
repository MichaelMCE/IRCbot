...
{
	// font height == 7
	irc->toppage->yoffset += FONT_LINEHEIGHT;
}
...
int renderText (TCHANNEL *channel, TFRAME *frame, char *buffer, int flags, int font)
{
	...
	rect.by2 += FONT_LINEHEIGHT;	// slight hack, 5x7.bdf font height = 7. make room for one extra line (see below)
